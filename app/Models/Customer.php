...
class Customer {
    ...
    public function create(array $d): int {
        $s = $this->db->prepare("INSERT INTO customers (business_id,name,company_name,branch,pan,vat_number,van_number,phone,email,address,credit_limit,opening_balance,payment_terms,status) VALUES (:bid,:name,:company,:branch,:pan,:vat,:van,:phone,:email,:address,:credit,:opening,:terms,:status)");
    ...
    public function update(int $id, array $d, int $bid = 0): bool {
        if ($bid === 0) $bid = $_SESSION['business_id'] ?? 1;
        $s = $this->db->prepare("UPDATE customers SET name=:name,company_name=:company,branch=:branch,pan=:pan,vat_number=:vat,van_number=:van,phone=:phone,email=:email,address=:address,credit_limit=:credit,payment_terms=:terms,status=:status WHERE id=:id AND business_id=:bid");
        return $s->execute(['name'=>$d['name'],'company'=>$d['company_name']??null,'branch'=>$d['branch']??null,'pan'=>$d['pan']??null,'vat'=>$d['vat_number']??null,'van'=>$d['van_number']??null,'phone'=>$d['phone']??null,'email'=>$d['email']??null,'address'=>$d['address']??null,'credit'=>$d['credit_limit']??0,'terms'=>$d['payment_terms']??0,'status'=>$d['status']??'active','id'=>$id,'bid'=>$bid]);
    }
