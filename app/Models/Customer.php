...
class Customer {
    ...
    public function create(array $d): int {
        ...
        $s->execute(['bid'=>$d['business_id']??($_SESSION['business_id']??1),'name'=>$d['name'],'company'=>$d['company_name']??null,'branch'=>$d['branch']??null,'pan'=>$d['pan']??null,'vat'=>$d['vat_number']??null,'phone'=>$d['phone']??null,'email'=>$d['email']??null,'address'=>$d['address']??null,'credit'=>$d['credit_limit']??0,'opening'=>$d['opening_balance']??0,'terms'=>$d['payment_terms']??0,'status'=>$d['status']??'active']);
