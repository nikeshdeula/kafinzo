...
class SalesStatement {
    ...
    public function getStatement(int $bid = 0, ?string $from = null, ?string $to = null, ?int $customer_id = null): array {
        ...
        $sql = "SELECT i.id, i.invoice_number AS ref_number, i.invoice_date AS date, c.name AS party_name, c.address AS party_address, c.branch AS party_branch, c.vat_number AS party_vat_number, i.subtotal, i.tax_amount, i.discount_amount, i.total_amount, 0 AS tds_amount, i.paid_amount, i.status, 'invoice' AS type FROM invoices i LEFT JOIN customers c ON i.customer_id=c.id WHERE i.business_id=:bid";
        $params = ['bid'=>$bid];
