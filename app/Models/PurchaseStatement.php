...
class PurchaseStatement {
    ...
    public function getStatement(int $bid = 0, ?string $from = null, ?string $to = null, ?int $supplier_id = null): array {
        ...
        $sql = "SELECT pb.id, pb.bill_number AS ref_number, pb.bill_date AS date, s.name AS party_name, s.address AS party_address, s.branch AS party_branch, s.vat_number AS party_vat_number, pb.subtotal, pb.tax_amount, pb.discount_amount, pb.total_amount, pb.paid_amount, pb.status, 'bill' AS type FROM purchase_bills pb LEFT JOIN suppliers s ON pb.supplier_id=s.id WHERE pb.business_id=:bid";
        $params = ['bid'=>$bid];
        ...
        $sql = "SELECT pp.id, pp.payment_number AS ref_number, pp.payment_date AS date, s.name AS party_name, s.address AS party_address, s.branch AS party_branch, s.vat_number AS party_vat_number, 0 AS subtotal, 0 AS tax_amount, 0 AS discount_amount, pp.amount AS total_amount, pp.amount AS paid_amount, 'completed' AS status, 'payment' AS type FROM purchase_payments pp LEFT JOIN suppliers s ON pp.supplier_id=s.id WHERE pp.business_id=:bid";
        $params = ['bid'=>$bid];
