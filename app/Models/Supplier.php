...
class Supplier {
    ...
    public function update(int $id, array $d): bool {
        $s = $this->db->prepare("UPDATE suppliers SET name=:name,company_name=:company,branch=:branch,pan=:pan,vat_number=:vat,phone=:phone,email=:email,address=:address,opening_balance=:opening,payment_terms=:terms,status=:status WHERE id=:id");
        return $s->execute(['name'=>$d['name'],'company'=>$d['company_name']??null,'branch'=>$d['branch']??null,'pan'=>$d['pan']??null,'vat'=>$d['vat_number']??null,'phone'=>$d['phone']??null,'email'=>$d['email']??null,'address'=>$d['address']??null,'opening'=>$d['opening_balance']??0,'terms'=>$d['payment_terms']??0,'status'=>$d['status']??'active','id'=>$id]);
    }
