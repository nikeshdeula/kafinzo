...
class Quotation {
    ...
    public function create(array $d): int {
        ...
        $s->execute(['bid'=>$d['business_id']??($_SESSION['business_id']??1),'cid'=>$d['customer_id'],'num'=>$d['quotation_number'],'qdate'=>$d['quotation_date'],'valid'=>$d['valid_until']??null,'sub'=>$d['subtotal']??0,'tax'=>$d['tax_amount']??0,'disc'=>$d['discount_amount']??0,'total'=>$d['total_amount']??0,'status'=>$d['status']??'draft','notes'=>$d['notes']??null]);
