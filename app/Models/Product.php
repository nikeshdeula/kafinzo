...
class Product {
    ...
    public function find(int $id): array|false {
        $s = $this->db->prepare("SELECT * FROM products WHERE id=:id LIMIT 1");
    ...
    public function update(int $id, array $d): bool {
        $s = $this->db->prepare("UPDATE products SET category_id=:cat,unit_id=:unit,name=:name,sku=:sku,type=:type,purchase_price=:pp,selling_price=:sp,tax_rate=:tax,opening_stock=:os,current_stock=:cs,minimum_stock=:ms,description=:desc,status=:status WHERE id=:id");
        return $s->execute(['cat'=>$d['category_id']??null,'unit'=>$d['unit_id']??null,'name'=>$d['name'],'sku'=>$d['sku']??null,'type'=>$d['type']??'product','pp'=>$d['purchase_price']??0,'sp'=>$d['selling_price']??0,'tax'=>$d['tax_rate']??0,'os'=>$d['opening_stock']??0,'cs'=>$d['current_stock']??$d['opening_stock']??0,'ms'=>$d['minimum_stock']??0,'desc'=>$d['description']??null,'status'=>$d['status']??'active','id'=>$id]);
    }

    public function delete(int $id): bool {
        $s = $this->db->prepare("DELETE FROM products WHERE id=:id");
