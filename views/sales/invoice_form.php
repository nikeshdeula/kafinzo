...
<div class="page-header d-flex align-items-center justify-content-between">
    <div>
        ...
        <p>Fill in the invoice details and line items.</p>
    </div>
    <a href="/sales/invoices" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i> Back</a>
</div>

<?php include BASE_PATH . 'views/layouts/alerts.php'; ?>

<div class="card transaction-form-card"><div class="card-body">
<?php $action = isset($invoice) ? '/sales/invoices/edit?id='.$invoice['id'] : '/sales/invoices/create'; ?>
<form action="<?= $action ?>" method="POST" id="invoiceForm">
    <div class="row g-3 mb-3 transaction-info">
        <div class="col-md-4">
            <label class="form-label fw-600">Invoice Number <span class="text-danger">*</span></label>
            <input type="text" name="invoice_number" class="form-control" required value="<?= htmlspecialchars($invoice['invoice_number'] ?? $invoiceNumber) ?>">
        </div>
        <div class="col-md-4">
            ...
            <select name="customer_id" class="form-select" required>
                ...
                <option value="<?= $c['id'] ?>" <?= (isset($invoice) && $invoice['customer_id']==$c['id']) ? 'selected' : '' ?>><?= htmlspecialchars($c['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-4">
            ...
            <select name="status" class="form-select">
                ...
                <option value="partial" <?= (isset($invoice) && $invoice['status']==='partial') ? 'selected' : '' ?>>Partial</option>
        ...
        <div class="col-md-4">
            ...
            <?= nepali_date_picker('invoice_date', $invoice['invoice_date'] ?? '', 'Invoice Date') ?>
        </div>
        <div class="col-md-4">
            <label class="form-label fw-600">Due Date</label>
            <?= nepali_date_picker('due_date', $invoice['due_date'] ?? '', 'Due Date') ?>
        </div>
    </div>
    ...
    <div class="table-responsive items-table-wrap mb-0">
        <table class="table table-bordered mb-0" id="itemsTable">
            ...
            <tbody id="itemsBody">
                <?php
                $items = isset($invoice['items']) && !empty($invoice['items']) ? $invoice['items'] : [['product_id'=>'','description'=>'','quantity'=>1,'unit_price'=>0,'discount_pct'=>0,'tax_rate'=>0,'amount'=>0]];
                ...
                <tr>
                    ...
                    <td><input type="number" name="items[<?= $idx ?>][tax_rate]" class="form-control item-tax" value="<?= htmlspecialchars($it['tax_rate'] ?: $taxRate) ?>" step="0.01" min="0" readonly></td>
    ...
    <div class="invoice-footer">
        ...
        <div class="col-12 d-flex gap-2 mt-2">
            <button type="submit" class="btn btn-primary"><i class="bi bi-save me-1"></i> Save Invoice</button>
            <a href="/sales/invoices" class="btn btn-outline-secondary">Cancel</a>
        </div>
    </div>
</form>
</div></div>
