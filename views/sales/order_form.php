...
<div class="page-header d-flex align-items-center justify-content-between">
    <div>
        ...
        <p><?= isset($order) ? 'Update sales order details.' : 'Record a new sales order for a customer.' ?></p>
    </div>
    <a href="/sales/orders" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i> Back</a>
</div>

<?php include BASE_PATH . 'views/layouts/alerts.php'; ?>

<div class="card transaction-form-card"><div class="card-body">
<?php $action = isset($order) ? '/sales/orders/edit?id='.$order['id'] : '/sales/orders/create'; ?>
<form action="<?= $action ?>" method="POST" id="orderForm">
    <div class="row g-3 transaction-info mb-3">
        ...
        <div class="col-md-3">
            ...
            <select name="customer_id" class="form-select" required>
                ...
                <option value="<?= $c['id'] ?>" <?= (isset($order) && $order['customer_id'] == $c['id']) ? 'selected' : '' ?>><?= htmlspecialchars($c['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-4">
            ...
            <select name="status" class="form-select">
                <?php foreach (['draft','pending','processing','shipped','delivered','cancelled'] as $st): ?>
    ...
    <div class="col-12 d-flex gap-2 mt-2">
        ...
        <a href="/sales/orders" class="btn btn-outline-secondary">Cancel</a>
    </div>
</form>
</div></div>
...
function calculateTotals() {
    ...
    document.querySelectorAll('#itemsTable tbody tr').forEach(row => {
        ...
        row.querySelector('.item-amount').textContent = 'NPR ' + (afterDisc + taxAmt).toFixed(2);
    });
